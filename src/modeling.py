...
#NOTE: problems when loading model on account of errors accessing relative path
...
# Constants used to get weights from originl ViT implementation (Tensorflow)
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = Linear(
            config.hidden_size, config.transformer["mlp_dim"]
        )
        self.fc2 = Linear(
            config.transformer["mlp_dim"], config.hidden_size
        )
...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            ...
            query_bias = np2th(
                weights[pjoin(ROOT, ATTENTION_Q, "bias")]
            ).view(-1)
            key_bias = np2th(
                weights[pjoin(ROOT, ATTENTION_K, "bias")]
            ).view(-1)
            value_bias = np2th(
                weights[pjoin(ROOT, ATTENTION_V, "bias")]
            ).view(-1)
            out_bias = np2th(
                weights[pjoin(ROOT, ATTENTION_OUT, "bias")]
            ).view(-1)
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            self.transformer.embeddings.cls_token.copy_(
                np2th(weights["cls"])
            )
            ...
            if self.transformer.embeddings.hybrid:
                ...
                for (
                    ...
                    for uname, unit in block.named_children():
                        unit.load_from(
                            weights, n_block=bname, n_unit=uname
                        )
...
class ViTransformer(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.config = config #(˶ᵔ ᵕ ᵔ˶)
        self.model = VisionTransformer(zero_head=True, num_classes=10)
    ...
    def configure_optimizers(self):
        ...
        if self.config.training.lr_scheduler.name == "cosine":
            ...
            raise ValueError(f"Unsupported Scheduler (｡•́︿•̀｡): {self.config.training.lr_scheduler.name}")
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            self.log("valid_loss", eval_loss, prog_bar=True)
            preds = torch.argmax(logits, dim=-1)
            preds = preds.detach().cpu().numpy()
            y = y.detach().cpu().numpy()
            acc = self.simple_accuracy(preds, y)
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        with torch.no_grad():
            logits = self.model(x)[0]
            test_loss = self.loss_fct(logits, y)
...
def get_config():
    ## This config matches the pretrained model.
